...
previous_losses = torch.tensor([0.0, 0.0, 0.0], device=device)
...
for epoch in range(wandb.config["Epochs"]):
    ...
    def closure():
        ...
        # Tensor of losses
        current_losses = torch.tensor([ic_loss, pde_loss, data_loss], device=device, requires_grad=True)

        # Total loss
        lambda_ = SoftAdapt(current_losses, previous_losses, beta=0.1)
        print(lambda_)

        loss = torch.dot(lambda_, current_losses)
        wandb.log({
            "Loss": loss.item(),

...
for epoch in range(wandb.config["Epochs"]):
    ...
    if epoch % 1000 == 0:
        with torch.no_grad():
            ...
            # Evaluate the model
            X_pred = f_known(t_eval.unsqueeze(-1).to(device))
            ...
            fig.add_scatter(x=t_eval, y=X_pred[:, 0].cpu().numpy(), mode='lines', name='Prey (pred)', line=dict(color='green'))
            fig.add_scatter(x=t_eval, y=X_pred[:, 1].cpu().numpy(), mode='lines', name='Predator (pred)', line=dict(color='red'))
            # Add datapoints
            ...
            # Plot missing terms
            res = f_unknown(X_pred).cpu().numpy()
            res_dx = res[:, 0]
            res_dy = res[:, 1]

...
for sample_size in [100, 500, 1000]:
    ...
    class NavierStokesUPINN(UPINN):
        ...
        def refine_collocation_points(self):
            ...
            self.collocation_points = RAD_sampler(Xc, residual, D, k, c).to('cpu')
    ...
    # model.softadapt_kwargs = dict(beta=0.2, loss_weigthed=True)

    adam = torch.optim.Adam(model.parameters(), lr=1e-3)
    scheduler = torch.optim.lr_scheduler.StepLR(adam, step_size=1000, gamma=0.9)
...
for sample_size in [100, 500, 1000, 2000]:
    name = 'ns-reverse-upinn-with-pressuredata-FNN64-sa02-RAD-alt-'+str(sample_size)
